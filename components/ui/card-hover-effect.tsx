      ...
      {items.map((item, idx) => {
        ...
        >
          <AnimatePresence>
            {hoveredIndex === idx && (
              <motion.span
                className="absolute inset-0 h-full w-full bg-neutral-900 dark:bg-slate-800/[0.8] block  rounded-lg"
          ...
          <div className="rounded-md w-full p-4 overflow-hidden bg-black group-hover:ring-2 ring-green-500 relative z-20 transition-all duration-500 cursor-pointer">
                <div className="py-10 z-50 relative space-y-5">
                    ...
                    <p className=" text-3xl font-bold text-center text-gray-300">{item.text}</p>
                </div>
          </div>
        </div>
        })}
